...
export function ServiceCamera({ serviceOrderId, onPhotoUploaded, onClose }: ServiceCameraProps) {
  ...
  if (!permission) {
    return <View style={styles.center}><ActivityIndicator color={colors.primaryContainer} /></View>;
  }

  if (!permission.granted) {
    return (
      <View style={styles.center}>
        <Text style={[styles.permText, { color: colors.onSurface }]}>Kamera erişimi gerekli</Text>
        <TouchableOpacity style={[styles.permBtn, { backgroundColor: colors.primaryContainer }]} onPress={requestPermission}>
    ...
    return (
      <View style={styles.preview}>
        ...
        <View style={[styles.previewActions, { backgroundColor: colors.surface }]}>
          <TouchableOpacity 
            style={[styles.retakeBtn, { borderColor: colors.outline }]} 
            onPress={() => setPhoto(null)}
          >
            <Text style={[styles.retakeBtnText, { color: colors.onSurface }]}>Yeniden Çek</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.uploadBtn, { backgroundColor: colors.primaryContainer }, uploading && styles.uploadBtnDisabled]}
            onPress={uploadPhoto}
            disabled={uploading}
          >
            {uploading ? (
...
const styles = StyleSheet.create({
  ...
  permText: { fontSize: 16 },
  permBtn: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 },
  ...
  retakeBtn: { flex: 1, borderWidth: 1, borderRadius: 12, padding: 14, alignItems: "center" },
  retakeBtnText: { fontWeight: "600" },
  uploadBtn: { flex: 1, borderRadius: 12, padding: 14, alignItems: "center" },
