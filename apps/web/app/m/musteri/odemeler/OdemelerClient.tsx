...
import { Banknote, CreditCard, Building2, Loader2, AlertCircle, ChevronRight } from "lucide-react";
...
export default function OdemelerClient() {
  ...
  const fetchPage = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/musteri/odemeler?page=${p}&limit=${LIMIT}`);
  ...
  return (
    <div className="space-y-5 pb-8">
      <div>
        <h1 className="text-2xl font-black text-[#00236f]">Ödeme Geçmişi</h1>
        <p className="text-sm text-gray-500 mt-0.5">{total} ödeme</p>
      </div>
