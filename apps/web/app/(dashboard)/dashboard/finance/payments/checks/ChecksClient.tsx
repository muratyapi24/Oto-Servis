...
export default function ChecksClient({ checks, upcomingCount }: { checks: any[]; upcomingCount: number }) {
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {checks.length === 0 ? (
                ...
                checks.map((check) => {
                  const statusCfg = STATUS_CONFIG[check.status as keyof typeof STATUS_CONFIG] ?? STATUS_CONFIG.PENDING;
                  ...
                  return (
                    <tr key={check.id} className={`hover:bg-slate-50/50 transition-colors ${isOverdue ? "bg-red-50/30" : isDueSoon ? "bg-amber-50/30" : ""}`}>
                      <td className="px-6 py-4 font-black text-slate-900">{check.checkNumber}</td>
                      <td className="px-6 py-4 text-slate-700">{getCustomerName(check.payment?.customer)}</td>
