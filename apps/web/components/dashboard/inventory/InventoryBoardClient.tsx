  ...
  return (
    <div className="flex-1 space-y-8 min-h-screen pb-12">
      ...
      <div className="space-y-3 mb-8">
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="relative flex-1 md:max-w-sm">
              ...
              <input 
                 ...
                 className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-gray-800 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300 outline-none transition-all shadow-sm" 
         ...
         <div className="flex flex-wrap items-center gap-2">
            <Link 
              ...
              className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 hover:text-blue-600 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-50 transition-all">
            ...
            {metrics.lowStockCount > 0 && (
              <button 
                ...
                className="flex items-center gap-1.5 text-red-500 hover:text-red-700 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-red-50 dark:bg-red-900/30 transition-all disabled:opacity-50 ml-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-2 bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl shadow-xl flex flex-col justify-between relative overflow-hidden text-white">
          ...
          <div className="z-10 relative">
             <span className="text-[10px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase">Toplam Stok (Depo) Değeri</span>
        ...
        {/* Categories Analysis */}
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
          <div>
            <span className="text-[10px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase">Stok Çeşitliliği</span>
            <div className="mt-3 flex items-end gap-3">
               <span className="text-3xl font-black text-slate-800 dark:text-gray-200">{metrics.totalPartsTypes}</span>
               <span className="text-sm font-bold text-slate-400 dark:text-slate-500 mb-1">Farklı Ürün</span>
            </div>
          </div>
          <div className="mt-6">
          ...
          metrics.lowStockCount > 0 
           ...
           <div>
              <div className="flex items-center justify-between">
                 <span className="text-[10px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase">Kritik Stok Uyarısı</span>
                 {metrics.lowStockCount > 0 ? (
              ...
              <div className="mt-3 flex items-end gap-3">
                 ...
                 <span className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Ürün</span>
              </div>
           </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 content-start h-full pb-8">
        ...
        {/* Full Inventory List */}
        <div className="xl:col-span-8 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 dark:bg-gray-800/50/50 text-slate-500 font-bold border-b border-slate-100">
                  <tr>
                  ...
                  {filteredParts.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-slate-400 dark:text-slate-500">
                        <PackageSearch className="w-10 h-10 mx-auto text-slate-300 dark:text-slate-600 mb-3" />
                    ...
                    filteredParts.map((p) => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-slate-50 dark:bg-gray-800/50/50:bg-slate-800/30 transition-colors">
                          <td className="px-6 py-4">
                            <div className="font-bold text-slate-900 dark:text-white mb-0.5">{p.name}</div>
                            <div className="flex items-center gap-2">
                               <div className="text-[10px] font-black tracking-widest text-slate-500 bg-slate-100 dark:bg-gray-700 px-2 py-0.5 rounded uppercase">
                                 {p.partNumber}
                               </div>
                               {p.brand && <span className="text-xs font-medium text-slate-400 dark:text-slate-500">{p.brand}</span>}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1.5 text-slate-700 dark:text-gray-300 font-medium text-xs mb-1">
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="relative inline-block text-left" ref={openDropdown === p.id ? dropdownRef : null}>
                              <button 
                                ...
                                className="text-slate-400 dark:text-slate-500 hover:text-amber-600 p-2 transition-colors rounded-xl hover:bg-amber-50"
                              >
                                <MoreVertical className="w-5 h-5 ml-auto" />
                              </button>
                              
                              {openDropdown === p.id && (
                                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-slate-200 rounded-xl shadow-lg z-50 py-1 overflow-hidden">
                                  ...
                                  <button 
                                    ...
                                    className="w-full text-left px-4 py-2 text-sm hover:bg-slate-50 dark:bg-gray-800/50 text-slate-700 flex items-center gap-2"
                                  >
                                  ...
                                  <button 
                                    ...
                                    className="w-full text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600 flex items-center gap-2 border-t border-slate-100 dark:border-gray-700"
                                  >
        ...
        <div className="xl:col-span-4 space-y-6">
           ...
           <div className="bg-red-600 text-white p-6 rounded-3xl shadow-lg relative overflow-hidden">
              ...
              <div className="mt-6 space-y-3 relative z-10">
                 {lowStockItems.length === 0 ? (
                    <div className="bg-white dark:bg-gray-800/10 p-4 rounded-xl text-xs font-bold text-center border border-white/10">
                       Kritik stokta ürün bulunmuyor.
                    </div>
                 ) : (
                    lowStockItems.slice(0, 4).map((item: any, idx) => (
                      <div key={idx} className="bg-white dark:bg-gray-800/10 p-3 rounded-xl border border-white/5 flex items-center justify-between">
                         <div>
                         ...
                         <button className="bg-white dark:bg-gray-800 text-red-700 hover:bg-red-50 text-[10px] font-black px-3 py-1.5 rounded-lg transition-colors">
                            SİPARİŞ
                         </button>
                      </div>
                    ))
           ...
           <div className="bg-white dark:bg-gray-800 border border-slate-100 p-6 rounded-3xl shadow-sm">
             <h5 className="text-xs font-black mb-6 uppercase tracking-widest text-slate-600 dark:text-slate-400 flex items-center gap-2">
             ...
             <div className="space-y-4">
               {recentMovements.length === 0 ? (
                 <p className="text-xs font-bold text-slate-400 dark:text-slate-500">Son zamanlarda stok hareketi kaydedilmemiş.</p>
               ) : (
                   ...
                   return (
                     <div key={idx} className="flex items-start gap-4 pb-4 border-b border-slate-50 last:border-0 last:pb-0">
                       ...
                       <div className="flex-1 min-w-0">
                         <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{mov.partName}</p>
                         <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">{mov.reason || 'Neden belirtilmemiş'} • {dayjs(mov.date).fromNow()}</p>
                       </div>
                       <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-2 py-1 bg-slate-50 dark:bg-gray-800/50 rounded">
                         {label}
                       </span>
                     </div>
                   );
             ...
             <button className="w-full mt-6 py-2.5 bg-slate-50 dark:bg-gray-800/50 text-slate-600 text-xs font-black rounded-xl hover:bg-slate-100:bg-slate-700 transition-colors uppercase tracking-widest">
               Tüm Hareketleri Gör
             </button>
           </div>
