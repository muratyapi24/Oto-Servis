...
function BarcodeScannerSkeleton() {
  return (
    <div className="w-full aspect-video bg-slate-100 rounded-2xl animate-pulse flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-black text-slate-900">Barkod ile Stok Girişi</h2>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6">
            ...
            <div className="space-y-4">
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-12">
                  ...
                  <span className="ml-3 text-sm font-bold text-slate-600">Parça aranıyor...</span>
                </div>
              ) : (
            ...
            <div className="space-y-5">
              ...
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-black text-slate-900 text-sm leading-tight">{foundPart.name}</h3>
                    <span className="inline-block mt-1 text-[10px] font-black tracking-widest text-slate-500 bg-slate-200 px-2 py-0.5 rounded uppercase">
                ...
                <div className="grid grid-cols-3 gap-3 pt-2 border-t border-slate-200">
                  <div className="text-center">
                    ...
                    <p className="text-xs font-black text-slate-900">{foundPart.currentStock}</p>
                    <p className="text-[10px] text-slate-500 font-medium">Mevcut Stok</p>
                  </div>
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-1 text-slate-400 mb-1">
                      <Tag className="w-3 h-3" />
                    </div>
                    <p className="text-xs font-black text-slate-900">{foundPart.unit}</p>
                    <p className="text-[10px] text-slate-500 font-medium">Birim</p>
                  </div>
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-1 text-slate-400 mb-1">
                      <BadgeDollarSign className="w-3 h-3" />
                    </div>
                    <p className="text-xs font-black text-slate-900">{formatMoney(foundPart.sellingPrice)}</p>
                    <p className="text-[10px] text-slate-500 font-medium">Satış Fiyatı</p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-700 uppercase tracking-widest">
                  Açıklama <span className="text-slate-400 font-medium normal-case">(isteğe bağlı)</span>
                </label>
                <input
                  type="text"
                  value={reason}
                  ...
                  className="w-full px-4 py-2.5 bg-slate-100 border-none rounded-xl text-sm focus:ring-2 focus:ring-amber-500/50 outline-none"
                />
              </div>
          ...
          {step === "part-not-found" && (
            <div className="space-y-5 text-center py-4">
              ...
              <div>
                <h3 className="font-black text-slate-900 text-sm">Parça Bulunamadı</h3>
                <p className="text-xs text-slate-500 mt-1">
                  <span className="font-bold text-slate-700">{scannedBarcode}</span> barkoduna sahip
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  type="button"
                  onClick={handleReset}
                  className="w-full py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-black hover:bg-slate-200 transition-colors"
                >
                  Tekrar Tara
                </button>
              </div>
          ...
          {step === "success" && (
            <div className="space-y-5 text-center py-4">
              ...
              <div>
                <h3 className="font-black text-slate-900 text-sm">Stok Girişi Başarılı</h3>
