...
export default function CriticalStockReport({ parts, summary }: CriticalStockReportProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Toplam Kritik</span>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-red-100 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Stok Tükendi</span>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Min. Altında</span>
          </div>
      ...
      {/* Tablo */}
      <div id="critical-stock-report-content" className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50">
          <h3 className="font-black text-slate-700 text-sm">Kritik Stok Listesi</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Parça</th>
                <th className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Kategori</th>
                <th className="text-right px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Mevcut</th>
                <th className="text-right px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Min. Stok</th>
                <th className="text-right px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Açık</th>
                <th className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Durum</th>
                <th className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">Tedarikçi</th>
              </tr>
            </thead>
            <tbody>
              {parts.length === 0 ? (
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right font-bold text-slate-600">{part.minStockLevel}</td>
                        <td className="px-4 py-3 text-right">
                          <span className="font-black text-red-600">-{part.deficit}</span>
                        </td>
                        <td className="px-4 py-3">
                          {isOutOfStock ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-red-100 text-red-700">
                        ...
                        <td className="px-4 py-3">
                          {part.supplier ? (
                            <button
                              ...
                              className="text-blue-600 hover:text-blue-700 font-medium text-sm underline-offset-2 hover:underline transition-colors"
                            >
                              {part.supplier.name}
                            </button>
                          ) : (
                      ...
                      {isExpanded && part.supplier && (
                        <tr key={`${part.id}-supplier`} className="border-b border-slate-50 bg-blue-50/30">
