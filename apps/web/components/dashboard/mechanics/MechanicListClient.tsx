    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex gap-2">
          <button className="px-4 py-2 text-xs font-bold bg-white text-slate-600 rounded-lg hover:bg-slate-50:bg-slate-800 transition-colors border border-slate-200 shadow-sm">
            Uzmanlık Filtresi
          </button>
          <button className="px-4 py-2 text-xs font-bold bg-white text-slate-600 rounded-lg hover:bg-slate-50:bg-slate-800 transition-colors border border-slate-200 shadow-sm">
            Verimliliğe Göre
      ...
      <div className="space-y-4">
        {initialMechanics.length === 0 ? (
          ...
          return (
            <div key={mechanic.id} className="group bg-white hover:bg-slate-50:bg-slate-800/80 p-5 rounded-3xl transition-all duration-300 flex items-center gap-6 shadow-sm border border-slate-200 relative overflow-hidden">
              ...
              <div className="relative pl-3">
                <div className="w-14 h-14 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xl font-black uppercase ring-4 ring-white shadow-sm transition-transform group-hover:scale-105">
              ...
              <div className="hidden md:block px-6 w-48">
                <div className="flex justify-between items-end mb-1.5">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Anlık İş Yükü</p>
              ...
              <div className="hidden sm:block text-right px-6 border-l border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Günlük Verim</p>
                <p className="text-[15px] font-black text-emerald-600 border border-emerald-100 bg-emerald-50 px-2.5 py-0.5 rounded-lg inline-block">
                  {efficiency}%
                </p>
              </div>
              
              <div className="flex gap-2">
                 <Link href={`/dashboard/mechanics/${mechanic.id}`} className="p-2.5 bg-slate-50 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors" title="Detay">
                   <Calendar className="w-4 h-4" />
                 </Link>
                 
                 {/* Interactions Group */}
                 <div className="flex gap-1 bg-slate-50 p-1 rounded-xl">
                   <button onClick={() => handleEdit(mechanic)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-white:bg-slate-700 rounded-lg transition-colors">
                     <Edit className="w-4 h-4" />
                   </button>
                   <button onClick={() => handleDeleteClick(mechanic)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-white:bg-slate-700 rounded-lg transition-colors">
