  ...
  return (
    <div className="flex-1 space-y-8 min-h-screen pb-12">
         ...
         <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-80">
              ...
              <input 
                 ...
                 className="w-full pl-10 pr-4 py-2 bg-slate-100 border-none rounded-xl text-sm focus:ring-2 focus:ring-blue-500/50 outline-none" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        {/* Daily Cash Flow */}
        <div className="md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between overflow-hidden relative group">
          <div className="z-10 relative">
             <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Günlük Nakit Girişi (Tahsilat)</span>
             <h3 className="text-4xl md:text-5xl font-black text-blue-700 mt-2 tracking-tighter">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col">
          <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Tahsilat Yaşlandırma</span>
        ...
        <div className="bg-slate-900 border border-slate-800 text-white p-6 rounded-2xl shadow-xl flex flex-col justify-between relative overflow-hidden">
           ...
           <div className="relative z-10">
              <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Entegrasyon Durumu</span>
              <div className="mt-5 space-y-4">
                 ...
                 <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-xs font-bold text-slate-300">Garanti (Hesap Hareketleri)</span>
                    </div>
                    <span className="text-[10px] font-medium text-slate-400">Senkronize Ediliyor...</span>
                 </div>
              </div>
           </div>
           <button className="mt-4 text-[9px] font-black underline text-slate-400 hover:text-white uppercase tracking-widest text-left relative z-10">APİ BAĞLANTISI EKLE</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 content-start h-full pb-8">
        ...
        <div className="xl:col-span-8 space-y-6">
           <div className="flex items-center justify-between">
              <h4 className="text-xl font-black tracking-tight text-slate-900">Tahsilat Bekleyen Faturalar</h4>
           </div>

           <div className="space-y-4">
             {filteredInvoices.length === 0 ? (
               <div className="p-10 border border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center text-center">
                 <Receipt className="w-12 h-12 text-slate-300 mb-3" />
                 <p className="text-sm font-bold text-slate-500">Tahsilat bekleyen faturanız bulunmuyor.</p>
               </div>
             ) : (
               filteredInvoices.map((inv: any) => {
                 ...
                 return (
                   <div key={inv.id} className="bg-white p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm hover:shadow-md transition-shadow border border-slate-100">
                     <div className="flex items-center gap-4">
                       <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${bgClass} ${iconColor}`}>
                         <Icon className="w-6 h-6" />
                       </div>
                       <div>
                         <Link href={`/dashboard/finances/invoices/${inv.id}`} className="text-sm font-black text-slate-900 hover:text-primary hover:underline">
                           {inv.invoiceNumber}
                         </Link>
                     ...
                     <div className="flex items-center gap-6 sm:gap-10 w-full sm:w-auto justify-between sm:justify-end">
                       <div className="text-right">
                         <span className="text-lg font-black block text-slate-900">{formatMoney(remainingInfo)}</span>
                         <span className={`text-[9px] uppercase font-black tracking-widest ${statusColor}`}>{statusBadge}</span>
                       </div>
                       <button 
                         onClick={() => setPaymentInvoice(inv)}
                         className="bg-slate-100 text-slate-700 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200:border-emerald-800 border-2 border-transparent border-dashed px-4 py-2 rounded-xl text-[11px] font-black tracking-widest flex items-center gap-2 transition-all">
                         TAHSİL ET
                       </button>
                     </div>
                   </div>
        ...
        <div className="xl:col-span-4 space-y-6">
           ...
           <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm">
             <h5 className="text-sm font-black mb-6 uppercase tracking-widest text-slate-600">Yaklaşan Gider/Ödemeler</h5>
             <div className="space-y-4">
             ...
             <button 
               ...
               className="w-full mt-8 py-3 bg-red-50 text-red-600 text-xs font-black rounded-xl hover:bg-red-100 hover:text-red-700 transition-colors uppercase tracking-widest">
