    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Toplam Satış</p>
          <p className="text-2xl font-black text-blue-700 mt-2">{formatMoney(totalSales)}</p>
          <p className="text-xs text-slate-500 mt-1">{invoices.filter(i => i.type === "SALES").length} fatura</p>
        </div>
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Toplam Alış</p>
          <p className="text-2xl font-black text-orange-600 mt-2">{formatMoney(totalPurchase)}</p>
          <p className="text-xs text-slate-500 mt-1">{invoices.filter(i => i.type === "PURCHASE").length} fatura</p>
        </div>
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-xs text-slate-500 mt-1">tamamlanmış fatura</p>
        </div>
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-xs text-slate-500 mt-1">ödeme bekleyen fatura</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="flex items-center gap-2 bg-white text-slate-600 border border-slate-200 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 transition-all"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Fatura No</th>
                <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Müşteri / Tedarikçi</th>
                <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tür</th>
                <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tarih</th>
                <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Vade</th>
                <th className="text-right px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tutar</th>
                <th className="text-right px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Ödenen</th>
                <th className="text-center px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Durum</th>
                <th className="text-center px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-6 py-16 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-400">Hiç fatura bulunamadı.</p>
                    <p className="text-xs text-slate-400 mt-1">Filtreleri değiştirin veya yeni bir fatura oluşturun.</p>
                  </td>
                </tr>
              ) : (
                filtered.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                      <td className="px-5 py-4">
                        <Link href={`/dashboard/finances/invoices/${inv.id}`} className="text-sm font-black text-slate-900 hover:text-blue-600 transition-colors">
                          {inv.invoiceNumber || "—"}
                        </Link>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <span className="text-xs font-medium text-slate-500">{dayjs(inv.issueDate).format("DD MMM YYYY")}</span>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-right">
                        <span className="text-sm font-black text-slate-900">{formatMoney(inv.totalAmount)}</span>
                      </td>
                      <td className="px-5 py-4 text-right">
