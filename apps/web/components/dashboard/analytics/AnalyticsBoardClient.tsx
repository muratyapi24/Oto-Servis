    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-2 justify-end mb-2">
        <button
          ...
          className="flex items-center gap-2 bg-white border border-outline-variant/30 text-on-surface px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-surface-dim transition-all"
        >
        ...
        <Link
          ...
          className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-800 transition-all"
        >
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-surface-container-highest p-6 rounded-2xl ambient-shadow border-b-4 border-primary">
          ...
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Aylık Gelir</p>
        </div>
        ...
        <div className="bg-surface-container-lowest p-6 rounded-2xl ambient-shadow border-b-4 border-secondary-container">
          ...
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Bu Ay Servis Emri</p>
        </div>
        ...
        <div className="bg-surface-container-lowest p-6 rounded-2xl ambient-shadow border-b-4 border-tertiary-container">
          ...
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Yeni Müşteri</p>
        </div>
        ...
        <div className="bg-surface-container-lowest p-6 rounded-2xl ambient-shadow border-b-4 border-primary">
          ...
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Tamamlanma Oranı</p>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Gelir Bar Chart */}
        <div className="lg:col-span-8 bg-white p-6 rounded-3xl ambient-shadow">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-on-surface">Aylık Gelir Trendi</h3>
            <span className="text-xs font-bold text-slate-400">Son 6 Ay</span>
          </div>
            ...
            {monthlyTrend.map((m, i) => {
              ...
              return (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-2">
                  <span className="text-[10px] font-black text-slate-500">{formatCurrency(m.revenue)}</span>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-surface-container-high p-6 rounded-3xl">
            ...
            <div className="flex items-end justify-between mt-2">
              ...
              <span className="text-xs font-bold text-slate-500">/ {formatCurrency(metrics.monthlyRevenue)}</span>
            </div>
            <div className="w-full bg-white h-2 rounded-full overflow-hidden mt-4">
              <div
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl ambient-shadow">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Güncel Akış</span>
          </div>

          {recentActivity.length === 0 ? (
            <div className="text-center py-8">
              <span className="material-symbols-outlined text-4xl text-slate-200 mb-2">history</span>
              <p className="text-sm text-slate-400">Henüz servis hareketi bulunmuyor.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-outline-variant/10">
                    <th className="text-left py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Plaka</th>
                    <th className="text-left py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Araç</th>
                    <th className="text-left py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Durum</th>
                    <th className="text-right py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Güncelleme</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline-variant/5">
                  {recentActivity.map((a) => (
                    <tr key={a.id} className="hover:bg-surface-container-low transition-colors">
                      ...
                      <td className="py-3 text-xs font-medium text-slate-600">{a.vehicleName}</td>
                      <td className="py-3">
                      ...
                      <td className="py-3 text-right text-[11px] text-slate-400 font-medium">
