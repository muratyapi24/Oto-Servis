...
export default function DashboardBoardClient({ data }: DashboardClientProps) {
  ...
  return (
    <>
      ...
      <div className="bg-primary text-white py-2 px-8 overflow-hidden whitespace-nowrap">
      ...
      <div className="p-8 space-y-8 max-w-7xl mx-auto">
        ...
        <section className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="space-y-2">
            <h2 className="text-slate-500 font-bold text-sm tracking-widest uppercase">Overview</h2>
            <h1 className="text-4xl font-bold text-on-surface tracking-tight">Merhaba {userName}! 👋</h1>
            <p className="text-slate-600 font-medium">
              Bugün yapılması gereken {metrics.activeServicesCount} aktif servis ve {appointments.length} yeni randevu var.
            </p>
          </div>
          <div className="flex space-x-3">
            <Link href="/dashboard/customers" className="flex items-center px-6 py-3 bg-white border border-outline-variant/20 rounded-xl text-primary font-bold text-sm hover:bg-blue-50 transition-all ambient-shadow">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-surface-container-highest p-6 rounded-2xl ambient-shadow border-b-4 border-primary">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-bold text-primary bg-primary/10 px-2 py-1 rounded">CANLI</span>
            </div>
            <p className="text-3xl font-black text-on-surface">{metrics.activeServicesCount}</p>
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Aktif Servisler</p>
          </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-2xl ambient-shadow border-b-4 border-tertiary-container">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-bold text-on-tertiary-fixed-variant bg-tertiary-fixed/30 px-2 py-1 rounded">TAMAMLANAN</span>
            </div>
            <p className="text-3xl font-black text-on-surface">{metrics.completedTodayCount}</p>
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Bugün Tamamlanan</p>
          </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-2xl ambient-shadow border-b-4 border-secondary-container">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-bold text-on-secondary-container bg-secondary-fixed/50 px-2 py-1 rounded">VADESİ GELEN</span>
            </div>
            <p className="text-3xl font-black text-on-surface">₺{metrics.pendingInvoicesTotal.toLocaleString("tr-TR")}</p>
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-1">Tahsil Edilecek</p>
          </div>

          {/* Stok Uyarıları */}
          <div className="bg-error-container p-6 rounded-2xl ambient-shadow border-b-4 border-error">
            <div className="flex justify-between items-start mb-4">
              <div className="p-2 bg-error/10 rounded-lg text-error">
                <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>inventory_2</span>
              </div>
              <span className="text-xs font-bold text-error bg-error/10 px-2 py-1 rounded">KRİTİK</span>
            </div>
            <p className="text-3xl font-black text-error">{metrics.lowStockCount}</p>
            <p className="text-[11px] font-bold text-error/70 uppercase tracking-wider mt-1">Stok Uyarıları</p>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-on-surface">Aktif Servis Kanvanı</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {/* Beklemede */}
              <div className="bg-surface-container-low p-3 rounded-2xl space-y-3 min-h-[400px]">
                <h4 className="px-2 py-1 text-[11px] font-black text-slate-500 uppercase tracking-tighter flex items-center border-b border-outline-variant/20 mb-4">
                  <span className="w-1.5 h-1.5 rounded-full bg-slate-400 mr-2"></span> Muayene Beklemede ({kanban.pending.length})
                </h4>
                {kanban.pending.map((order: any) => (
                  <div key={order.id} className="bg-white p-4 rounded-xl ambient-shadow space-y-3 cursor-pointer hover:ring-2 ring-primary/20 transition-all">
                    <div className="flex justify-between items-start">
                      <span className="text-[10px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-bold uppercase truncate max-w-[120px]">
                    ...
                    <div>
                      <p className="text-sm font-bold">{order.vehicle.plate}</p>
                      <p className="text-xs text-slate-500 font-medium">{order.vehicle.brand} {order.vehicle.model}</p>
                    </div>
                    {order.assignedMechanic && (
                      <div className="flex items-center space-x-2">
                        ...
                        <p className="text-[10px] text-slate-400">{order.assignedMechanic.firstName} {order.assignedMechanic.lastName}</p>
                      </div>
                    )}
                  </div>
                ))}
                {kanban.pending.length === 0 && <p className="text-xs text-slate-400 text-center pt-8">Boş</p>}
              </div>

              {/* İşlemde */}
              <div className="bg-surface-container-low p-3 rounded-2xl space-y-3 min-h-[400px]">
                <h4 className="px-2 py-1 text-[11px] font-black text-secondary-container uppercase tracking-tighter flex items-center border-b border-outline-variant/20 mb-4">
                  <span className="w-1.5 h-1.5 rounded-full bg-secondary-container mr-2"></span> İşlemde ({kanban.inProgress.length})
                </h4>
                {kanban.inProgress.map((order: any) => (
                  <div key={order.id} className="bg-white p-4 rounded-xl ambient-shadow space-y-3 ring-2 ring-secondary/10">
                    <div className="flex justify-between items-start">
                      <span className="text-[10px] bg-orange-50 text-orange-700 px-2 py-0.5 rounded font-bold uppercase truncate max-w-[120px]">
                ...
                {kanban.inProgress.length === 0 && <p className="text-xs text-slate-400 text-center pt-8">Boş</p>}
              </div>

              {/* Test Ediliyor */}
              <div className="bg-surface-container-low p-3 rounded-2xl space-y-3 min-h-[400px]">
                <h4 className="px-2 py-1 text-[11px] font-black text-blue-700 uppercase tracking-tighter flex items-center border-b border-outline-variant/20 mb-4">
                ...
                {kanban.testing.map((order: any) => (
                  <div key={order.id} className="bg-white p-4 rounded-xl ambient-shadow space-y-3 border-l-4 border-blue-500">
                    <p className="text-sm font-bold">{order.vehicle.plate}</p>
                    <p className="text-xs text-slate-500 font-medium">{order.vehicle.brand} {order.vehicle.model}</p>
                ...
                {kanban.testing.length === 0 && <p className="text-xs text-slate-400 text-center pt-8">Boş</p>}
              </div>

              {/* Teslime Hazır */}
              <div className="bg-surface-container-low p-3 rounded-2xl space-y-3 min-h-[400px]">
                <h4 className="px-2 py-1 text-[11px] font-black text-tertiary-container uppercase tracking-tighter flex items-center border-b border-outline-variant/20 mb-4">
                  <span className="w-1.5 h-1.5 rounded-full bg-tertiary-container mr-2"></span> Teslime Hazır ({kanban.readyCount})
                </h4>
                {kanban.readyCount > 0 ? (
                  <div className="bg-tertiary-fixed/10 p-4 rounded-xl space-y-3 border border-tertiary-container/20">
                    <p className="text-sm font-bold text-center">{kanban.readyCount} araç teslime hazır</p>
                    <Link href="/dashboard/services" className="block w-full py-2 bg-tertiary-container text-white text-[10px] font-bold rounded-lg uppercase text-center">
                      Fatura Kes
                    </Link>
                  </div>
                ) : (
                  <p className="text-xs text-slate-400 text-center pt-8">Boş</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-white p-6 rounded-3xl ambient-shadow">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold">Bugünkü Randevular</h3>
                <span className="text-xs font-bold text-slate-400">{getTodayLabel()}</span>
              </div>
              {appointments.length > 0 ? (
                    ...
                    return (
                      <div key={app.id} className="relative pl-10">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-sm font-bold">{app.type || "Araç Kabul Başvurusu"}</p>
                          <p className="text-xs text-slate-500">Müşteri: {customerName}</p>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-slate-400">Bugün randevu bulunmuyor.</p>
                </div>
              )}
            </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Son Aktiviteler */}
          <div className="lg:col-span-1 bg-white p-6 rounded-3xl ambient-shadow">
                ...
                {recentActivities.slice(0, 5).map((activity) => (
                  <li key={activity.id} className="flex items-start space-x-3">
                    ...
                    <div className="text-xs">
                      <span className="font-bold text-on-surface">{activity.description}</span>
                      <p className="text-slate-400 mt-0.5">{activity.timeAgo}</p>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-surface-container-high p-6 rounded-3xl flex flex-col justify-between">
              ...
              <div className="flex items-end justify-between">
                <p className="text-2xl font-black">{formatCurrency(finance.weeklyRevenue)}</p>
                <span className={`text-xs font-bold ${finance.revenueChangePercent >= 0 ? "text-tertiary-container" : "text-error"}`}>
              ...
              <div className="w-full h-12 mt-4 flex items-end space-x-1">
                {finance.revenueBarHeights.map((h, i) => (
                  <div
                    key={i}
                    className={`flex-1 rounded-t ${i === finance.revenueBarHeights.length - 1 ? "bg-primary" : "bg-primary/20"}`}
            ...
            <div className="bg-surface-container-high p-6 rounded-3xl flex flex-col justify-center items-center text-center">
              <div className="relative w-24 h-24 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle className="text-secondary" cx="48" cy="48" fill="transparent" r="40" stroke="currentColor" strokeDasharray="251.2" strokeDashoffset={calcDashOffset(stats.successRate).toString()} strokeWidth="8"></circle>
                </svg>
                <span className="absolute text-lg font-black text-on-surface">{stats.successRate}%</span>
              </div>
              <p className="mt-3 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Başarı Oranı</p>
            </div>
            ...
            <div className="bg-surface-container-high p-6 rounded-3xl flex flex-col justify-between">
              <div>
                ...
                <p className="text-3xl font-black mt-1">{stats.totalCustomers}</p>
              </div>
              <div className="flex text-secondary-container mt-4">
                <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>group</span>
                <span className="text-xs font-bold text-slate-500 ml-2 self-center">Kayıtlı Müşteri</span>
              </div>
              <p className="text-[10px] font-medium text-slate-400 mt-2">Aktif müşteri portföyü.</p>
            </div>
          </div>
        </section>
      </div>
